        ...
        if jmin > 0:
            for i in range(jmin):
                if isinstance(grid, UniformNonPeriodicGrid):
                    prob_i = np.arange(self.dof)[:, None]
                    prob_j = self.j + jmin - i
                    S = 1/factorial(prob_i)*(prob_j*self.dx)**prob_i

                    b = np.zeros( self.dof )
                    b[self.derivative_order] = 1.

                    new_stencil = np.linalg.solve(S, b)
                    matrix[i,:self.dof] = new_stencil
                else:
        ...
        if jmax > 0:
            for i in range(jmax):
                if isinstance(grid, UniformNonPeriodicGrid):
                    ...
                    new_stencil = np.linalg.solve(S, b)
                    matrix[-i-1,-self.dof:] = new_stencil
                else:
